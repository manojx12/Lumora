...
function CapabilityRow({ capability, onSelect }: { capability: Capability; onSelect: () => void }) {
  ...
  return (
    <button type="button" onClick={onSelect} className="w-full" {...hoverBindings}>
      <animated.div
        style={rowStyle}
        className="flex items-center gap-4 rounded-card-sm py-6 text-left sm:gap-6 sm:py-8"
      >
      ...
      <Shell className="px-5 py-20 sm:px-8 lg:py-28">
        ...
        <ul>
          {CAPABILITIES.map((capability, index) => (
            ...
            >
              <CapabilityRow capability={capability} onSelect={openModal} />
            </Reveal>
          ))}
        </ul>
      </Shell>
