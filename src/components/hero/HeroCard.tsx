...
import { HERO_CARD_ITEMS } from '../../data/content';
...
export function HeroCard() {
  ...
  const go = (step: number) => {
    ...
    setIndex((current) => (current + step + HERO_CARD_ITEMS.length) % HERO_CARD_ITEMS.length);
  };
  ...
  return (
    <div className="w-full max-w-96 rounded-card-sm bg-white/70 p-2 shadow-sm ring-1 ring-line/70 backdrop-blur-md lg:w-76">
      ...
      <div
        onClick={() => go(1)}
        className="flex w-full cursor-pointer gap-2 rounded-control text-left"
      >
        ...
        <div className="flex flex-1 flex-col justify-between rounded-control bg-surface/70 p-3">
          <div className="relative min-h-13" aria-live="polite">
            {HERO_CARD_ITEMS.map((item, itemIndex) => (
              <CarouselItem
                key={item.caption}
          ...
          <div className="mt-2 flex items-center justify-between gap-3">
            <div className="flex items-center gap-1" aria-hidden="true">
              {HERO_CARD_ITEMS.map((item, dotIndex) => (
                <span
                  key={item.caption}
...
function CarouselItem({
  ...
  return (
    ...
    >
      ...
      <p className="max-w-32 text-sm leading-[1.35] font-medium">{title}</p>
    </animated.div>
  );
}
